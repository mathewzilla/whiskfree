% Touch variables
% For a given mouse, load and plot variables at time of touch
clear all
mouse = 32;
...
load(['~/Dropbox/Data/3posdata/licks_',num2str(mouse),'.mat']);

behav = eval(['behav_',num2str(mouse)]);
meta = eval(['meta_',num2str(mouse)]);
touch = eval(['touch_',num2str(mouse)]);
licks = eval(['licks_',num2str(mouse)]);

clear(['behav_',num2str(mouse)]);
clear(['meta_',num2str(mouse)]);
clear(['touch_',num2str(mouse)]);
clear(['licks_',num2str(mouse)]);
...
for i = 1:numel(behav)
    ...
    if ~isempty(tracked)
        ...
        for j = 1:numel(touched)
            ...
            tt(n) = behav{i}.trialtype(touched(j));                 % trialtype
            ch(n) = behav{i}.choice(touched(j));                    % choice
            sess(n) = i;                                            % session
...
%             touchnames = {'No Touch','Protract','Retract'};
% %             choicename = {'Posterior','Middle','Anterior'};% 36
%             choicename = {'Middle','Posterior','Anterior'};% 32
...
%             
%             % Licking
%             lick_time = behav{i}.poleup(touched(j)) + l(n,find(l(n)));
%             if lick_time
%                 plot([lick_time,lick_time],[75,80],'c','linewidth',2)
%             end
%             
%             hold off;
%             title(['TT:',choicename{tt(n)},', Ch:',choicename{ch(n)},', Touch:',touchnames{tch(n)+1},' (trial ',num2str(touched(j)),')'])
%             xlim([815,2500])
%             pause
            ...
            % Hilbert stuff. Compute set point at pole up. Phase at touch
...
% Confusion matrices of touch/notouch
...
for i = 1:3;
    ...
    % Plot median on top
    plot(median(th_tch(these_ts)),90,'*','color',colours(i,:),'markersize',10); % was 80
...
%     title('Max delta kappa peri touch')
    ...
    % First touch time
    xtt = linspace(0,2000,201); % [~,xtt] = hist(fst_t(find(fst_t))-pu(find(fst_t)),100); 
...
for i = 1:2;
    ...
    corr_lick = find(l(these_ts,i))%3-i)); % Swap lick direction for 32,33,34
...
%     ntt./sum(ntt)
    ...
    xft = linspace(0,2000,201); % [~,xft] = hist(fst_t(touch_trials)-pu(touch_trials),450); % 
...
%     % Correct Lick (left or right)
%     [nl] = histc(851 + l(these_ts(corr_lick),i),linspace(500,2500,50));
%     xl = linspace(500,2500,50);
%     subplot(3,1,2);
%     bar(xl,nl,'facecolor',colours(i,:)); hold all
%     % Plot median on top
%     plot(median(851 + l(these_ts(corr_lick),i)),50,'*','color',colours(i,:));
%     
%     % Incorrect lick
%     [nl] = histc(851 + l(these_ts(incorr_lick),3-i),linspace(500,2500,50));
%     xl = linspace(500,2500,50);
%     subplot(3,1,2);
%     bar(xl,nl,'facecolor',0.5*(1+colours(i,:))); hold all
%     % Plot median on top
%     plot(median(851 + l(these_ts(incorr_lick),3-i)),50,'*','color',0.5*(1+colours(i,:)));
%     title('Lick time')
